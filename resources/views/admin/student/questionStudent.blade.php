...
<body class="bg-gray-100">
    ...
    <div id="modal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-6 w-1/3 relative">
            ...
            <label for="studentElementId" class="block text-sm font-medium text-gray-700">Elemen Mahasiswa</label>
            <select id="studentElementId" class="border border-gray-300 p-2 w-full mb-4">
    ...
    <div id="editModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-6 w-1/3 relative">
            ...
            <label for="editStudentElementId" class="block text-sm font-medium text-gray-700">Elemen Mahasiswa</label>
            <select id="editStudentElementId" class="border border-gray-300 p-2 w-full mb-4">
                <!-- Options akan diisi dengan JavaScript -->
            </select>
    ...
    <script>
        ...
        // Fungsi untuk mengambil student elements
        async function fetchStudentElements() {
            ...
            if (response.ok) {
                const data = await response.json();
                const studentElementSelect = document.getElementById('studentElementId');
                const editStudentElementSelect = document.getElementById('editStudentElementId');
                studentElementSelect.innerHTML = ''; // Kosongkan opsi sebelumnya
                editStudentElementSelect.innerHTML = ''; // Kosongkan opsi sebelumnya
                data.data.forEach(element => {
                    studentElementSelect.innerHTML += `<option value="${element.id}">${element.name}</option>`;
                    editStudentElementSelect.innerHTML += `<option value="${element.id}">${element.name}</option>`;
                });
            } else {
                alert('Gagal mengambil data elemen mahasiswa');
            }
        }
        ...
        document.getElementById('saveElement').onclick = async function() {
            ...
            const response = await fetch(`${BASE_URL}/api/admin/student-questions`, {
                ...
                body: JSON.stringify({
                    ...
                    max_range: maxRange,
                    label,
                    student_element_id: studentElementId
                })
            });
        ...
        document.getElementById('updateElement').onclick = async function() {
            ...
            const studentElementId = document.getElementById('editStudentElementId').value;

            const response = await fetch(`${BASE_URL}/api/admin/student-questions/${currentQuestionId}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
        ...
        // Panggil fungsi fetchQuestions dan fetchStudentElements saat halaman dimuat
