...
<head>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch('/api/admin/filled-acadstaff-questionnaires', {
                method: 'GET',
                headers: {
                    ...
                    data.data.forEach((item, index) => {
                        ...
                        row.innerHTML = `
                            ...
                            <td class="py-2 px-4 border-b">
                                <strong>Nama: </strong> ${item.acad_staff_name || 'N/A'}<br>
                                <strong>Kuesioner: </strong> ${item.answers.length > 0 ? item.answers[0].acad_staff_questionnaire : 'N/A'}<br>
                                
                                <ul>
                                    ${item.answers && Array.isArray(item.answers) ? item.answers.map(answer => `
                                        <li>
                                            <strong>Elemen: </strong> ${answer.acad_staff_element || 'N/A'}<br>
                                            <strong>Pertanyaan: </strong> ${answer.acad_staff_question || 'N/A'}<br>
