...
<body class="bg-gray-100">
    ...
    <div id="modal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-6 w-1/3 relative">
            ...
            <label for="lecturerElementId" class="block text-sm font-medium text-gray-700">Elemen Dosen</label>
            <select id="lecturerElementId" class="border border-gray-300 p-2 w-full mb-4">
    ...
    <div id="editModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-6 w-1/3 relative">
            ...
            <label for="editLecturerElementId" class="block text-sm font-medium text-gray-700">Elemen Dosen</label>
            <select id="editLecturerElementId" class="border border-gray-300 p-2 w-full mb-4">
                <!-- Options akan diisi dengan JavaScript -->
            </select>
    ...
    <script>
        ...
        // Fungsi untuk mengambil lecturer elements
        async function fetchLecturerElements() {
            ...
            if (response.ok) {
                const data = await response.json();
                const lecturerElementSelect = document.getElementById('lecturerElementId');
                const editLecturerElementSelect = document.getElementById('editLecturerElementId');
                lecturerElementSelect.innerHTML = ''; // Kosongkan opsi sebelumnya
                editLecturerElementSelect.innerHTML = ''; // Kosongkan opsi sebelumnya
                data.data.forEach(element => {
                    lecturerElementSelect.innerHTML += `<option value="${element.id}">${element.name}</option>`;
                    editLecturerElementSelect.innerHTML += `<option value="${element.id}">${element.name}</option>`;
                });
            } else {
        ...
        document.getElementById('saveElement').onclick = async function() {
            ...
            const response = await fetch(`${BASE_URL}/api/admin/lecturer-questions`, {
                ...
                body: JSON.stringify({
                    ...
                    max_range: maxRange,
                    label,
                    lecturer_element_id: document.getElementById('lecturerElementId').value // Pastikan ini diisi
                })
            });
        ...
        document.getElementById('updateElement').onclick = async function() {
            ...
            const lecturerElementId = document.getElementById('editLecturerElementId').value;
            ...
            const response = await fetch(`${BASE_URL}/api/admin/lecturer-questions/${currentQuestionId}`, {
                ...
                body: JSON.stringify({
                    ...
                    max_range: maxRange,
                    label,
                    lecturer_element_id: lecturerElementId
                })
            });
        ...
        // Panggil fungsi fetchQuestions dan fetchLecturerElements saat halaman dimuat
        document.addEventListener('DOMContentLoaded', async () => {
            await fetchQuestions();
            await fetchLecturerElements();
        });
